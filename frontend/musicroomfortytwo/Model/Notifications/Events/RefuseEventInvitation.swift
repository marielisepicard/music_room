...
class RefuseEventRequest {
    
    static let shared = RefuseEventRequest()
    private init() {}
    private var task: URLSessionTask?
    
    func refuseEventRequest(eventId: String, friendId: String, callback: @escaping (Int) -> Void) {
        let route = UserDefaults.standard.string(forKey: "route")!
        let userId = UserDefaults.standard.string(forKey: "userId")!
        let url = URL(string: route + "/users/" + userId + "/events/" + eventId + "/refuseInvitation/friends/" + friendId)!
