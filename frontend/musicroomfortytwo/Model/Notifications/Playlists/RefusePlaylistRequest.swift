...
class RefusePlaylistRequest {
    
    static let shared = RefusePlaylistRequest()
    private init() {}
    private var task: URLSessionTask?
    
    func refusePlaylistRequest(playlistId: String, callback: @escaping (Int) -> Void) {
        let route = UserDefaults.standard.string(forKey: "route")!
        let userId = UserDefaults.standard.string(forKey: "userId")!
        let url = URL(string: route + "/users/" + userId + "/playlists/" + playlistId + "/refuseInvitation")!
