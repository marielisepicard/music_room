...
class AcceptFriendRequest {
    ...
    static let shared = AcceptFriendRequest()
    private init() {}
    private var task: URLSessionTask?
    
    func acceptFriendRequest(friendId: String, callback: @escaping (Int) -> Void) {
        let route = UserDefaults.standard.string(forKey: "route")!
        let userId = UserDefaults.standard.string(forKey: "userId")!
        let url = URL(string: route + "/users/" + userId + "/friends/" + friendId + "/acceptInvitation")!
