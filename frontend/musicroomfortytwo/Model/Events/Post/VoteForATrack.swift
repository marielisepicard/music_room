...
extension Event {
    func voteForATrack(trackId: String, userLoc: CLLocationCoordinate2D?, callback: @escaping (Int) -> Void) {
        var task: URLSessionTask?
        let route = UserDefaults.standard.string(forKey: "route")!
        var rootUrl = route + "/events/" + self.id!
        rootUrl += "/tracks/" + trackId + "/vote"
                ...
                if let responseJSON = try? JSONDecoder().decode([String: String].self, from: data) {
                    if let response = response as? HTTPURLResponse {
                        if response.statusCode == 200 {
                            ...
                            if responseJSON["code"] == "2" { //User must be located near to the event
                                callback(2)
                                return
